...
int main (void) {
    ...
    P3M1 &= ~0x20;
    P3M0 |= 0x20;
    /* Set P4.1, P4.2, P2.0, P2.1 as high-impedance */
    P2M1 |= 0x03;
    P2M0 &= ~0x03;
    P4M1 |= 0x06;
    P4M0 &= ~0x06;
    /* Set Encoder port P3.2 as quasi bidirectional */
    P3M1 &= ~0x04;
    P3M0 &= ~0x04;
...
void TM1_ISR_Handler(void) interrupt(TM1_VECTOR) {
    ...
    /* Next Option(P4.2) Last Option(P2.1) SETTING(P4.1) OptionsSel(P2.0), these pins are pull up high when idle.
    ...
    // P2.0
    read1 = (P2 & 0x01) ^ 0x01;         // Read P2.2 and check if key is pressed. Was key pressed, read1 would be 1.
